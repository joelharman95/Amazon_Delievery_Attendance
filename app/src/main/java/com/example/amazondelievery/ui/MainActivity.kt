package com.example.amazondelievery.ui
...
import com.example.amazondelievery.ui.home.dashboard.BackPressEvent
...
class MainActivity : AppCompatActivity() {
    ...
    fun isSupervisor(isSupervisor: Boolean) {
        if (!isSupervisor) {
            bottomNavHome.menu.removeItem(R.id.employeeFrag)
            bottomNavHome.menu.removeItem(R.id.dayDeliveryFrag)
            bottomNavHome.menu.removeItem(R.id.leaveRequestFrag)
        }
    }
    ...
    override fun onBackPressed() {
        if (findNavController(R.id.credentialNavHost).currentDestination?.id != null) {
            when (findNavController(R.id.credentialNavHost).currentDestination?.id) {
                ...
                R.id.splashFragment, R.id.personalDetailsFrag, R.id.loginFragment, R.id.taskFrag, R.id.relaxFrag, R.id.employeeFrag, R.id.dayDeliveryFrag, R.id.leaveRequestFrag -> {
                    finish()
                }
